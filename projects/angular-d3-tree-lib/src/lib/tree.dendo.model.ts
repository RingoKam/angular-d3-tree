      ...
      .attr("d", d3.symbol()
        ...
        .type(function (d) {
          if ((d.children && d.children.length > 0) || (d._children && d._children.length > 0)) {
            return d3.symbolCross   //"cross";
          } else {
            return d3.symbolCircle  //"diamond";
          }
        }));
  ...
  dragBehaviour() {
    ...
    function dragEnd(d) {
      ...
      if (treeModel.selectedNodeByDrag) {
        ...
        // Make sure that the node being added to is expanded so user can see added node is `cor`rectly moved
